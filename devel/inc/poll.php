...
elseif (($action=="display") && (isset($_GET['poll'])))
{
	...
	$totalVotes = query("SELECT * FROM pollVoted WHERE pollID = '".escape_string($poll)."'");
	$totalVotes = num($totalVotes);
	echo $row->text;

	$queryV = query("SELECT votes FROM pollA WHERE QID = '".escape_string($poll)."'");	// get how many votes has been made on this poll
	...
	$queryU = query("SELECT * FROM pollVoted WHERE userID = '".escape_string($uid)."' AND pollID = '".escape_string($poll)."'");	// create a query that finds out whether this user has voted before or not on this poll.
	...
	$queryV = query("SELECT * FROM pollA WHERE QID = '".escape_string($poll)."'");	// retrieve all the alternatives.
...
elseif (($action == "castvote") && (isset($_GET['AID'])))
{
	...
	$q = query("SELECT * FROM pollA WHERE AID = '".escape_string($AID)."'");
	$qo = fetch($q);
	$qid = $qo->QID;
	
	$maxQ = query("SELECT * FROM pollQ WHERE ID = '".escape_string($qid)."'");
	$maxR = fetch($maxQ);
	$maxVote = $maxR->maxVotes;

	$userQ = query("SELECT * FROM pollVoted WHERE userID = '".escape_string($uid)."' AND pollID = '".escape_string($qid)."'");
	$userNUM = num($userQ);
	...
	query("UPDATE pollA SET votes = votes +1 WHERE AID = '".escape_string($AID)."'");
	query("INSERT INTO pollVoted SET userID = '".escape_string($uid)."', pollID = '".escape_string($qid)."'");
