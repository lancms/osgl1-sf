...
if(!isset($action)) {
	$q = query("SELECT * FROM Clan WHERE moderator = '".mysql_escape_string(getcurrentuserid())."'");
	if(num($q) == 0)
	{
		echo $clan[3];
	}
...
elseif($action == "addclan")
{
	...
	query("INSERT INTO Clan SET name = '".mysql_escape_string($name)."',
			password = '".mysql_escape_string($password)."',
			about = '".mysql_escape_string($about)."',
			moderator = '".mysql_escape_string(getcurrentuserid())."'");
	echo $form[40];
	echo $clan[6];
...
elseif($action == "edit" && isset($_GET['edit']))
{
	...
	$q = query("SELECT * FROM Clan WHERE ID = '".mysql_escape_string($edit)."'");
	$r = fetch($q);
...
elseif($action == "doedit" && isset($_GET['edit']))
{
	...
	$password = $_POST['password'];
	$about = $_POST['about'];
	$test = query("SELECT * FROM Clan WHERE name LIKE '".mysql_escape_string($name)."' AND ID != '".mysql_escape_string($edit)."'");
	if(num($test) != 0)
	{
		nicedie($clan7);
	}

	query("UPDATE Clan SET name = '".mysql_escape_string($name)."',
			password = '".mysql_escape_string($password)."',
			about = '".mysql_escape_string($about)."'
