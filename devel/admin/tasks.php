...
if(!isset($action))
{
	...
	$q = query("SELECT * FROM tasks ORDER BY userID = '".escape_string(getcurrentuserid())."' DESC, userID = 1");
	if(num($q) != 0)
	{
		echo "<table>";
...
{
	$name = $_POST['name'];
	$exists = query("SELECT * FROM tasks WHERE name = '".escape_string($name)."'");
	if (num($exists) != 0)
	{
		nicedie(lang("Sorry, but the task has already been added.", "admin_tasks", "Task-error: task already added").refresh("admin.php?adminmode=tasks", 2));
	}

	query("INSERT INTO tasks SET name = '".escape_string($name)."'");
...
{
	$q = query("SELECT * FROM tasks WHERE ID = '".escape_string($edit)."'");
	$r = fetch($q);
	...
	$q2 = query("SELECT * FROM tasks_log WHERE taskID = '".escape_string($edit)."'");
...
elseif (($action == "addcomment") && (isset($edit)))
{
	...
	query("INSERT INTO tasks_log SET userID = '".escape_string(getcurrentuserid())."', taskID = '".escape_string($edit)."', logUNIX = '".escape_string(time())."', logText = '".escape_string($comment)."'");
